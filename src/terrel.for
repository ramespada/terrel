...
c-----------------------------------------------------------------------
c
c --- TERREL    Version: 7.0.0           Level: 141010             MAIN
c 
c     Copyright (c) 1994-2014 by Exponent, Inc.
c
c
...
c --- UPDATES:
c
c --- Ver. TNG-3.8.0 to Ver. 7.0.0 Level 141010
c              - Update system documentation to Version 7
c                Control file dataset updated to 7.0
c                Output TERREL.DAT dataset version remains 2.0
c                (no format change)
c
c --- Ver. TNG-3.7.1 to Ver. TNG-3.8.0 Level 141010
c              - Trap and report error opening files (not found)
c                                                       (D. Strimaitis)
c                NEW:      OPEN_ERR (in CALUTILS v7.0.0)
c                MODIFIED: SETUP, SETARM3, SET3SEC, SETGENER, SETGLOB,
c                          SETSRTM, SETUSGSG, RDUSGSHD, COASTEXT, 
c                          SETGEOTIFF
c              - Revise processing for high resolution coastline
c                dataset GSHHG header (starting with v2.0)
c                                                       (D. Strimaitis)
c                MODIFIED: BREADH
c              - Revise test for logical LCDED
c                                                        (J. Popovic)
c                MODIFIED: LOADSTRP, LOADOTHER
c
c --- Ver. 3.7.0 Level 140829 to Ver. TNG-3.7.1 Level 140912
c              - The high resolution Canadian terrain data (CDED) are
c                in the sheets smaller than 1 degree by 1 degree. These 
c                data are provided in sheets of 0.25 by 0.25 degrees. 
c                This version of the code reads all DEM files as 
c                1 deg x1 deg, except for the CDED high resolution data 
c                that are read as 0.25 deg x 0.25 deg. Three new 
c                variables are introduced in DBINF.TRL file. 
c                                                        (J. Popovic)
c                Modified: RDUSGSHD, LOADOTHER, LOADSTRP, DBINF.TRL
c             -  CALUTILS updated from v2.58 to v2.6.0 to incorporate
c                F95 system routines for date-time and command-line
c                input
c                                                       (D. Strimaitis)
c
c --- Ver. 3.69 Level 110330 to Ver. 3.7.0 Level 140829
c                                                       (C. Escoffier)
c              - Fixed bug in calling SETRANO with an integer UTM zone
c                (izone) that is alway positive.  Instead, SETRANO
c                needs to be called with the signed integer (iutm) that
c                is negative in the S. Hem.  This had resulted in an
c                error in S. Hem. UTM coordinates from GeoTIFF datasets.
c                Modified: LOADTIFF
c
c --- Ver. 3.687 Level 090511 to Ver. 3.69 Level 110330
c                                                       (D. Strimaitis)
c              - CALUTILS from v2.571 Level 090511 to v2.58 Level 110225
c                Add control file variable type 5 (character array)
c                and retain commas in string returned so that array
c                of variable values can be parsed
c                Modified: READIN, ALTONU, SETVAR
c              - Correct list-file unit in call to COORDSVER
c                Modified: READCF
c              - Add lat/lon near/far strip coordinates for direct
c                conversion of Geo coordinates (lat,lon) to (x,y)
c                Modified: LOADOTHER, LOADSTRP, LOADSRTM
c              - Add control-file option MSHEET to use original mapping
c                of Geo coordinates to Cartesian coordinates in
c                LOADOTHER, or to do the direct conversion of each Geo
c                coordinate using COORDS transformation
c                Modified: CONTROL.TRL
c                          BLOCK DATA, READCF, SETUP, LOADOTHER
c              - Fix near-far calculation in LOADSRTM that was changed in
c                Draft (revert to earlier calculation)
c                Modified: LOADSRTM
c                --------------------------------
c              - Add GeoTIFF input data formats         (K. Morrison)
c              - Skip any empty polygons in newer versions of GSHHS
c                since these are simply lines (COASTEXT)
c              - Change treatment of Lat-Lon files in LOADOTHER so that
c                the coordinates of each datapoint are transformed
c              - Support both HGT (big-endian) and BIL (small-endian)
c                forms of SRTM data (SETSRTM and LOADSRTM, and add a
c                a logical to DBINF.TRL)
c              - Add support for GeoTIFFS coming from the Seamless
c                USGS server and other sources (new routines SETGEOTIFF
c                and LOADTIFF, add a logical to CONTROL.TRL, add 
c                TIFFINFO.FOR library, modify READCF, SETUP, COMP)
c              - Add support for the CDED DEMs (only requires the default
c                datum since these are LL DEMs in the USGS format)
c                (SETUP, READCF, COMP, LOADOTHER, LOADSTRP)
c              - Add TMSCALE as an input in Group 2 (READCF) and apply
c                it (SETUP, SPAN, GET4CNR, SETTRANO)
c              - Add support for the latest version of GSHHS (BREADH)
c
c                New     : SETGEOTIFF, LOADTIFF
c                          TIFFINFO.FOR Version: 1.03 (090123) ---
c                          get_ifd, read1b, read12b, read1c, read2b,
c                          read4b, read24b, read4r, read8r, readtiff
c
c                Modified: DBINF.TRL, CONTROL.TRL, GRID.TRL
c                          BLOCK DATA, SETUP, READCF, SPAN, COMP
c                          SETSRTM, RDUSGSHD, LOADOTHER, LOADSTRP,
c                          LOADSRTM, GET4CNR, SETTRANO, BREADH,
c                          COASTEXT
c
c --- Ver. 3.686 Level 080407 to Ver. 3.687 Level 090511
...
c----------------------------------------------------------------------
c
c --- TERREL    Version: 7.0.0         Level: 110330       BLOCK DATA
c               D. Strimaitis
c
...
c --- Primary variables
      ...
      data lrawecho/.false./
      data imodel/1/, igrid/1/, iproc/2/, msheet/0/
      data pmap/'UTM     '/
      data datum/'WGS-84'/
      ...
      data dsrtm1/'WGS-96'/, dsrtm3/'WGS-96'/, dcded/'NAR-C'/
      data dgeotiff/'WGS-84'/, dwvs/'WGS-84'/, dwdbii/'WGS-72'/
c --- Derived variables
      ...
      data lcded/.false./, lgeotiff/.false./
...
c --- GRID common block
      ...
      data feast/0.0/, fnorth/0.0/, tmscaleo/1.0/
...
c-----------------------------------------------------------------------
c
c --- TERREL    Version: 7.0.0         Level: 141010             SETUP
c               D. Strimaitis
c
c
...
c
c --- Updates
c     Ver 3.69 to Ver TNG-3.8.0 Level 141010
c              - Trap and report error opening files (not found)
c     Ver 3.686 Level 060202 to Ver 3.69 Level 110330  KAM
c              - Added CDED and GeoTIFF support
c              - Added TMSCALE
c              - Added MSHEET                          DGS
...
c --- Set text for reporting input options
     ...
     &            'NUATMOS','Generic'/
      ...
      data csheet
     &   /'Interpolate from sheet corners (TERREL Version < 3.69)',
     &    'Direct Transformation          (Preferred)            '/
...
c --- Open the control file
      open(iocnt,file=runinp,status='old',iostat=ierr)
...
c --- Open remaining files (not terrain DB files)
     ...
     &        iostat=ierr)
         if(ierr.NE.0) call OPEN_ERR(iolst,'SETUP','PREVIOUS File',
     &                               prevfil,ioprev)
      endif
      if(LXY) then
         open(ioxyi,file=xyinp,status='old',iostat=ierr)
         if(ierr.NE.0) call OPEN_ERR(iolst,'SETUP','XY Data File',
     &                               xyinp,ioxyi)
...
c -----------------------------
      ...
      write(iolst,*)
      write(iolst,*) 'For Data Base (DB) files with data points'
      write(iolst,*) 'distributed in latitude-longitude -------'
      write(iolst,*) 'DB Lat-Lon to x-y :  ' ,csheet(msheet+1)
      write(iolst,*)
...
c      write(iolst,*) '  - pond on island  = ',terdef(5),', ',iterrep(5)
     ...
     &       'dbfile   : ',datatyp(i),datafil(i)
      ...
      write(iolst,*) 'CDED   : ',dcded
      write(iolst,*) 'GEOTIFF: ',dgeotiff
     ...
     &    coastbln
      ...
      if(LTTM) write(iolst,*) 'tmscale    : ' ,tmscaleo
      if(LPOLR)then
...
c----------------------------------------------------------------------
c
c --- TERREL    Version: 7.0.0         Level: 110330            READCF
...
c --- UPDATES:
c     Ver 3.686 Level 060202 to Ver 3.69 Level 110330       KAM
c              - Add support for CDED and GeoTIFF files
c              - Add TMSCALE as an input in Group 2
c              - Correct list-file unit number in COORDSVER call  (DGS)
c              - Add MSHEET option to control data sheet extraction
c                method                                           (DGS)
...
c           tmscaleo,izone,rlat,rlon,xlat1,xlat2,icode(4),
...
c --- Dimension CTEMP(132, MAX(names,ntypes)), DBTYPE(ntypes)
      character*4 ctemp(132,15)
      character*6 dbtype(13)
      character*132 dbfil
      character*50 verdoc
...
c --- Set Dictionary
     ...
     2  'NZGEN','GEN','SRTM1','SRTM3','CDED','GEOTIFF',  47*' ',
...
c          4 = character
     ...
     2  13*4, 47*0,
     3  15*4, 45*0,
     b  3,3,3,3,2,1,2,2,1,2,1,2,3,3,1,2, 44*0,
     c  4,2,6*4,2,1,1,2,2,1,3*2,3*1, 40*0,
     d  1, 59*0,
     2  1, 59*0/
...
c ---    Read one DB filename entry (one of 13 DB types)
...
c ---       Identify type of DB file by first character
            ...
            elseif(ctemp(j,11)(1:1).ne.' ') then
               ii=11
               lsrtm3=.TRUE.
            elseif(ctemp(j,12)(1:1).ne.' ') then
               ii=12
               lcded=.TRUE.
            elseif(ctemp(j,13)(1:1).ne.' ') then
               ii=13
               lgeotiff=.TRUE.
            elseif(ii.EQ.0) then
...
c --- Transfer the char*4 data into the char*8 variables
      ...
      if(ctemp(1,14)(1:1).ne.' ') then
         do j=1,8
            dcded(j:j)=ctemp(j,14)(1:1)
         enddo
      endif
      if(ctemp(1,15)(1:1).ne.' ') then
         do j=1,8
            dgeotiff(j:j)=ctemp(j,15)(1:1)
         enddo
      endif
c      write(*,*) dgeotiff
...
c -----------------
     ...
     2 INOISEREPW,TERDEFW,ITERREPW,LCOAST,LBLNREAD,CELLRADKM,MSHEET,
...
c --- Initialize input false easting and northing and TM scaling 
c --- with defaults
      feastin=feast
      fnorthin=fnorth
      tmscale=tmscaleo
...
c --- Transfer input false easting and northing and scaling
c --- if the projection can use it
      if(LLCC.or.LTTM.or.LLAZA) then
         feast=feastin
         fnorth=fnorthin
         if(LTTM) tmscaleo=tmscale
      endif
...
c --- Test for valid MSHEET
      if(msheet.LT.0 .OR. msheet.GT.1) then
         write(iolst,*)
         write(iolst,*) 'READCF:  Error in Input Group 1'
         write(iolst,*) 'MSHEET out of range      = ',msheet
         write(iolst,*) 'MSHEET should be 0 or 1'
         lerrcf=.TRUE.
      endif
...
c-----------------------------------------------------------------------
c
c --- TERREL    Version: 7.0.0          Level: 110330              SPAN
c               D. Strimaitis, E. Insley
c
...
c
c --- Update
c     Ver 3.686 Level 050817 to Ver 3.69 Level 110330 KAM
c              Remove TMSONE, since the scale factor can now be input
c              and is in the GRID common as TMSCALEO
c              Assign dummy values for GLOBE1 calls (DGS)
...
c ---    Using Tangential TM grid
         call GLOBE1('TM      ',idum,tmscaleo,xdum,xdum,rlat,rlon,
     &               feast,fnorth,
     &               'LL      ',idum,xdum,xdum,xdum,rdum,rdum,
     &               feasti,fnorti,
     &               caction,vecti,vecto)
...
c-----------------------------------------------------------------------
c
c --- TERREL    Version: 7.0.0         Level: 040311          FINDFILE
...
c     logical lwarn,lexist
      ...
      character filenm1*2,filenm2*3,filenm3*12,globtmp*7
      character cn*1,cs*1,ce*1,cw*1
      character*1 clcded(4,4)
      character*19 filenme

      data cn/'n'/, cs/'s'/, ce/'e'/, cw/'w'/
      data clcded/'A','B','C','D','H','G','F','E','I','J','K',
     & 'L','P','O','N','M'/
c     data lwarn/.FALSE./
...
c        lexist=.FALSE.
         ...
         write(filenm2,'(i3.3)') IABS(lonse(is))
         if(latse(is).LT.0) then
            if(lonse(is).LT.0) then
               filenme=filenm1//cs//filenm2//ce//filenm3
            else
               filenme=filenm1//cs//filenm2//cw//filenm3
            endif
         else
            if(lonse(is).LT.0) then
               filenme=filenm1//cn//filenm2//ce//filenm3
            else
               filenme=filenm1//cn//filenm2//cw//filenm3
            endif
         endif
...
c ---    For SRTM, the name is the SW corner    
         ...
         if(latse(is).LT.0) then
            if((lonse(is)+1).LT.0) then
               filenme=cs//filenm1//ce//filenm2//filenm3
            else
               filenme=cs//filenm1//cw//filenm2//filenm3
            endif
         else
            if((lonse(is)+1).LT.0) then
               filenme=cn//filenm1//ce//filenm2//filenm3
            else
               filenme=cn//filenm1//cw//filenm2//filenm3
            endif
         endif
...
c --- Check if CDED files are within the domain
      if(.not.((maxx.lt.51.or.minx.gt.141).or.maxy.lt.41)) then
c ---   Write block names for CDED data to list file
      write(iolst,*)
      write(iolst,*) 'Data from the following CDED blocks are needed:'

        ifrstx=((minx-48)/8)
        ilastx=((maxx-48)/8)
        ifrsty=((miny-40)/4)
        ilasty=((maxy-40)/4)
        ibottom=mod(miny,4)+1
        itop=mod((maxy-1),4)+1
        ileft=mod(minx,8)/2+1
        iright=mod((maxx-1),8)/2+1
        do iouterx=ifrstx,ilastx
          if(iouterx.ne.ifrstx) ileft=1
          irtemp=4
          if(iouterx.eq.ilastx) irtemp=iright 
          do ioutery=ifrsty,ilasty
            ibtemp=1
            if(ioutery.eq.ifrsty) ibtemp=ibottom
            ittemp=4
            if(ioutery.eq.ilasty) ittemp=itop
            do innerx=ileft,irtemp
              do innery=ibtemp,ittemp
                write(filenme,fmt='(i2.2,i1,a1)') iouterx,ioutery,
     &            clcded(innerx,innery)
              enddo
            enddo
          enddo
        enddo

c ---    Write CDED filename to LIST file
         write(iolst,*) filenme
...
c----------------------------------------------------------------------
c
c --- TERREL    Version: 7.0.0         Level: 110330              COMP
c               D. Strimaitis
c
...
c
c --- Updates
c --- Ver 3.686 Level 031031 to Ver 3.69 Level 110330  KAM
c              Add support for CDED and GeoTIFF DBs
...
c ---     USGS 7.5 minute file                         ---  call RDUSGSHD
        ...
        elseif(datatyp(k) .EQ.'CDED'.OR. datatyp(k) .EQ. 'cded')then
c ---     CDED USGS-type DEM file                      ---  call RDUSGSHD
          l7_5min = .FALSE.
          l3sec = .FALSE.
          lcded= .TRUE.
          call RDUSGSHD(k)
...
c ---     SRTM 3-sec terrain dataset (DEM)             ---  call SETSRTM
        ...
        elseif(datatyp(k) .EQ.'GEOTIF' .OR. datatyp(k) .EQ.'geotif')
     &  then
c ---     GeoTIFF file                                 ---  call SETGEOTIFF
          call SETGEOTIFF(k)
          dbdatum=dgeotiff
...
c-----------------------------------------------------------------------
c
c --- TERREL    Version: 7.0.0         Level: 050824          SAVEELEV
...
c-----------------------------------------------------------------------
c
c --- TERREL    Version: 7.0.0         Level: 000411          FINDGLOB
...
c-----------------------------------------------------------------------
c
c --- TERREL    Version: 7.0.0         Level: 141010           SETARM3
...
c
c --- Update
c     Ver 3.65 to Ver TNG-3.8.0 Level 141010
c              - Trap and report error opening files (not found)
...
c --- Local variables
      ...
      open(ioinp,file=datafil(k),status='OLD',iostat=ierr)
      if(ierr.NE.0) call OPEN_ERR(iolst,'SETARM3','Data File',
     &                            datafil(k),ioinp)
...
c-----------------------------------------------------------------------
c
c --- TERREL    Version: 7.0.0         Level: 141010           SET3SEC
...
c CALLING ROUTINES:   COMP
c
c EXTERNAL ROUTINES:  CORNERS, OPEN_ERR
c-----------------------------------------------------------------------
c --- Include file of parameters and commons
      include 'params.trl'
      include 'control.trl'
...
c    &             access=caccess)
     ...
     &           access=cd3access,iostat=ierr)
      if(ierr.NE.0) call OPEN_ERR(iolst,'SET3SEC','Data File',
     &                            datafil(k),ioinp)
...
c-----------------------------------------------------------------------
c
c --- TERREL    Version: 7.0.0         Level: 141010          SETGENER
c               K. Morrison, Earth Tech
c
...
c
c --- Update
c     Ver 3.63 to Ver TNG-3.8.0 Level 141010
c              - Trap and report error opening files (not found)
c
c
c INPUT:
...
c --- Include file of parameters and commons
      ...
      lfeet=.FALSE.
      open(ioinp,file=datafil(k),status='OLD',iostat=ierr)
      if(ierr.NE.0) call OPEN_ERR(iolst,'SETGENER','Data File',
     &                            datafil(k),ioinp)
...
c-----------------------------------------------------------------------
c
c --- TERREL    Version: 7.0.0         Level: 141010           SETGLOB
c               J. Chang, Earth Tech
c
...
c    &           access='direct',recl=240)
     ...
     &           access='direct',recl=240,iostat=ierr)
      if(ierr.NE.0) call OPEN_ERR(iolst,'SETGLOB','Data File',
     &                            datafil(k),ioinp)
...
c-----------------------------------------------------------------------
c
c --- TERREL    Version: 7.0.0          Level: 141010           SETSRTM
c               K. Morrison
c
...
c
c --- Updates
c     Ver 3.687 to Ver TNG-3.8.0 Level 141010
c              - Trap and report error opening files (not found)
c --- Ver. 3.686 Level 050817 to Ver. 3.687_draft Level 110330   KAM
c              Allow both big-endian (.HGT) and small-endian (.BIL) sets
c --- Ver. 3.65 Level 050815 to Ver. 3.66 Level 050817   KAM
...
c GNU Compiler
      inquire(file=datafil(k),exist=lexist,size=isize)
...
c --- Check that file exists
     ...
     &   lexist) then 
        nxdb=1 ! longitude
        nydb=1 ! latitude
        lensrtmnm=len_trim(justname(k))
        srtmext=justname(k)(lensrtmnm-2:lensrtmnm)
        if(srtmext.eq.'HGT'.or.srtmext.eq.'hgt') lsrtmend=.TRUE. 
      else
...
c
      ...
      write(iolst,*) 'Is the machine platform "big endian?" ',lbigendian
      write(iolst,*) 'Is file "big endian?" ',lsrtmend
c
c --- Check if the grid area is completely outside of the assigned
...
c-----------------------------------------------------------------------
c
c --- TERREL    Version: 7.0.0         Level: 141010          SETUSGSG
...
c CALLING ROUTINES:   COMP
c
c EXTERNAL ROUTINES:  GET4CNR, OPEN_ERR
c-----------------------------------------------------------------------
c --- Include file of parameters and commons
      include 'params.trl'
      include 'filnam.trl'
...
c    &           access='transparent')
      open(ioinp,file=datafil(k),status='old',form=cform,
     &           access=caccess, recl=irecl,iostat=ierr)
      if(ierr.NE.0) call OPEN_ERR(iolst,'SETUSGSG','Data File',
     &                            datafil(k),ioinp)
...
c-----------------------------------------------------------------------
c
c --- TERREL    Version: 7.0.0          Level: 141010          RDUSGSHD
c               D. Strimaitis
...
c     Ver TNG-3.7.1 to Ver TNG-3.8.0 Level 141010
c              - Trap and report error opening files (not found)
c     Ver 3.687 Level 110330 to Ver. TNG-3.7.1 Level 140912 (J.Popovic)
c              Add support for high resolution Canadian CDED data:
c              allocate horizontal resolution and 
c              assign real longitude and latitude of SE corner of DB file
c     Ver 3.686 Level 061109 to 3.687_draft Level 110330  KAM
c              Add support for CDED DBs
...
c CALLING ROUTINES:   COMP
c
c EXTERNAL ROUTINES:  SETTRANO, GLOBE, OPEN_ERR
c-----------------------------------------------------------------------
c --- Include file of parameters and commons
      include 'params.trl'
      include 'control.trl'
...
c --- Local variables
      ...
      character*6 chdatum(4)/'NAS-C','WGS-72','WGS-84','NAR-C'/
...
c --- For coordinate transformations
     ...
     &          cnrlatsw,cnrlonsw,cnrlatnw,cnrlonnw
      ...
      lgeotiff = .FALSE.
      open(ioinp,file=datafil(k),status='OLD',iostat=ierr)
      if(ierr.NE.0) call OPEN_ERR(iolst,'RDUSGSHD','Data File',
     &                            datafil(k),ioinp)
...
c --- Set input dataset false Easting/Northing to zero
      ...
      write(iolst,*)
      write(iolst,*) '-------------------------------'
      write(iolst,*) 'USGS/CDED Data File Information'
      write(iolst,*) '-------------------------------'
      write(iolst,*)

c --- Read header record of DB terrain file
      read(ioinp,1000) title,icode,adum,ixunit,izunit,nside,
     &                 qcornr,zmnmx,rdum,iacc,rdelta,nprof,ihdatum
1000  format(a144,4i6,15d24.15,3i6,11d24.15,i6,3e12.6,2i6,26x,i2)
      if(ihdatum.gt.0.and.ihdatum.le.4) then
        dbdatum = chdatum(ihdatum)
      else
        dbdatum = dusgs90
      endif
...
c --- 9/9/14 fix: allocate horizontal resolution (for Canadian high res data)
      hor_res=rdelta(2)
...
c ---     Set translation vectors to the output grid
            call SETTRANO('UTM     ',icode(4),xdum,xdum,xdum,xdum,xdum,
     &                  xdum,feasti,fnorti,
     &                  iutm,tmscaleo,xlat1,xlat2,rlat,rlon,
     &                  feast,fnorth,
     &                  caction,vecti,vecto)
...
c ----------------------------------------------------
c ---   *** USGS and CDED lat/lon degree-based DEM ***
c ----------------------------------------------------
...
c ---   9/9/14 fix: assign real longitude and latitude of SE corner of DB file
        alonsedb=-qcornr(7)/3600.
        alatsedb=qcornr(8)/3600.
        xsw=-qcornr(1)/3600.
        ysw=qcornr(2)/3600.
        xnw=-qcornr(3)/3600.
        ynw=qcornr(4)/3600.
...
c ---   Strip information (number of profiles and points/profile)
        nstr=nprof(2)
        npt=nprof(2)
c ---   3-secstep = 1200 steps/degree  (use RDELTA info)
        stepa = rdelta(2)/3600.
        stepb = rdelta(1)/3600.
...
c ---   Check if the terrain data in file overlaps the grid area
        ...
        minys=MIN(yse,ysw)
        if(maxx .LT. minxs .OR. minx .GT. maxxs) then
          write(iolst,*)'RDUSGSHD: CDED or USGS 90m Data file NOT',
     &    ' in designated grid'
          lnotgrid=.TRUE.
        elseif(maxy .LT. minys .OR. miny .GT. maxys) then
          write(iolst,*)'RDUSGSHD: CDED or USGS 90m Data file NOT',
...
c-----------------------------------------------------------------------
c
c --- TERREL    Version: 7.0.0         Level: 051201           CORNERS
c               D. Strimaitis, SRC
c
...
c --- Set translation vectors
      call SETTRANO('LL      ',idum,xdum,xdum,xdum,xdum,xdum,
     &              feasti,fnorti,
     &              iutm,tmscaleo,xlat1,xlat2,rlat,rlon,
     &              feast,fnorth,
     &              caction,vecti,vecto)
...
c-----------------------------------------------------------------------
c
c --- TERREL    Version: 7.0.0         Level: 061113          LOADDMDF 
c               K. Morrison, Earth Tech
c
c
...
c --- Set translation vectors to the output grid
      iutm11=11
      iutm12=12
      call SETTRANO('UTM     ',iutm11,xdum,xdum,xdum,xdum,xdum,
     &             feasti,fnorti,
     &             iutm,tmscaleo,xlat1,xlat2,rlat,rlon,
     &             feast,fnorth,
     &             caction,vecti11,vecto11)
      call SETTRANO('UTM     ',iutm12,xdum,xdum,xdum,xdum,xdum,
     &             feasti,fnorti,
     &             iutm,tmscaleo,xlat1,xlat2,rlat,rlon,
     &             feast,fnorth,
...
c-----------------------------------------------------------------------
c
c --- TERREL    Version: 7.0.0         Level: 061113         LOADNZGNR
c               K. Morrison, Earth Tech
c
c
...
c-----------------------------------------------------------------------
c
c --- TERREL    Version: 7.0.0         Level: 051201           LOADGNR
c               K. Morrison, Earth Tech
c
c
...
c --- Set translation vectors to the output grid
     ...
     &               caction,vecti,vecto)
         ...
         call SETTRANO('UTM     ',izoneipt,xdum,xdum,xdum,xdum,xdum,
     &               feasti,fnorti,
     &               iutm,tmscaleo,xlat1,xlat2,rlat,rlon,
     &               feast,fnorth,
     &               caction,vecti,vecto)
...
c-----------------------------------------------------------------------
c
c --- TERREL    Version: 7.0.0         Level: 051201          LOADLAZA 
c               K. Morrison, Earth Tech
c
c
...
c --- Set translation vectors to the output grid
      call SETTRANO('LAZA    ',idum,rlati,rloni,xdum,xdum,xdum,
     &             feasti,fnorti,
     &             iutm,tmscaleo,xlat1,xlat2,rlat,rlon,
     &             feast,fnorth,
     &             caction,vecti,vecto)
...
c-----------------------------------------------------------------------
c
c --- TERREL    Version: 7.0.0          Level: 140912         LOADOTHER
c               K. Morrison
c
c
...
c
c --- Update
c --- Ver 3.69 Level 110330 to Ver TNG-3.7.1 Level 140912 (J.Popovic)
c              Add support for high resolution Canadian CDED data:
c              get the real value of the DB index 
c --- Ver 3.686 Level 051201 to Ver 3.69 Level 110330  KAM
c              Change all Lat-Lon files so that the coordinates for
c              each data point are transformed instead of interpolated
c              to avoid distortions at higher latitudes
c ---                                                  DGS
c              Revise method to do lat-lon interpolations in LOADSTRP
c              and toggle method with new MSHEET control
...
c ---    Calculate the DB index
         lon = ii - 1 + lonsedb
         lat = jj - 1 + latsedb
c ---    9/9/14 fix: get the real value of the DB index
         atlon = ii - 1 + alonsedb
         atlat = jj - 1 + alatsedb
         ndx = (jj-1)*nxdb + ii
c
...
c ---    Also get lat-lon of first point in file
         if(.not.L7_5MIN) then
c ---    9/9/14 fix: real values of the DB index already used for Canadian
c        high resolution data                
           if(lcded.and.(hor_res.eq.0.75)) then
             alon = atlon
             alat = atlat
           else
             alon = FLOAT(lon)
...
c ---       (strip goes from near to far edge of sheet)
c ---                                                ---  call LOADSTRP
            call LOADSTRP(j,lat,lon,rdata,xnear,ynear,xfar,yfar,
     &                                    xlnear,ylnear,xlfar,ylfar)

c --- debug
            if(j.EQ.1 .OR. j.EQ.nstr) then
               write(iolst,*)'LOADSTRP: j,lat,lon = ',j,lat,lon
               write(iolst,*)'xnear,ynear,xfar,yfar = ',
     &                        xnear,ynear,xfar,yfar
               write(iolst,*)'xlnear,ylnear,xlfar,ylfar = ',
     &                        xlnear,ylnear,xlfar,ylfar
            endif
...
c ---          Latitude, Longitude (degrees)
               delxl = (xlfar-xlnear)*stepb
               delyl = (ylfar-ylnear)*stepa
            endif
...
c ---             Translate UTM coordinates to grid
                  xkm=(xnear + delx * im1)*0.001
                  ykm=(ynear + dely * im1)*0.001
                  call GLOBE(iolst,caction,dbdatum,vecti,datum,vecto,
     &                       xkm,ykm,xkmo,ykmo,idum,c4dum)
                  x=xkmo*1000.
                  y=ykmo*1000.
               elseif(msheet.EQ.0) then
c ---             Interpolate coordinates from corner (x,y)
                  x = xnear + delx * im1
                  y = ynear + dely * im1
               elseif(msheet.EQ.1) then
c ---             Interpolate Lat-Lon coordinates
c ---             Note:  Interpolated West Long converted to East Long
                  x_lon = -(xlnear + delxl * im1)
                  y_lat = ylnear + delyl * im1
c ---             Transform to (x,y)
                  call GLOBE(iolst,caction,dbdatum,vecti,datum,vecto,
     &                       x_lon,y_lat,xkmo,ykmo,idum,c4dum)
                  x=xkmo*1000.
...
c-----------------------------------------------------------------------
c
c --- TERREL    Version: 7.0.0          Level: 140912          LOADSTRP
c               D. Strimaitis
c               J. Chang
c
c
...
c --- Ver 3.69 Level 110330 to Ver TNG-3.7.1 Level 140912 (J.Popovic)
c              Add support for high resolution Canadian CDED data:
c              Read 0.25 by 0.25 degree sheets for high res CDED data
c              and 1 by 1 degree sheets for other DEM data
c     Ver 3.686 Level 050817 to Ver 3.69 Level 110330   KAM
c              Add CDED processing to the L3SEC processing 
c              Add latitude/longitude near-far output  DGS
c     Ver 3.65 Level 050815 to Ver 3.66 Level 050817   KAM
...
c             xlnear    Longitude of point at start of strip
c                       (positive for western hemisphere)
c             ylnear    Latitude of point at start of strip
c                       (positive for northern hemisphere)
c             xlfar     Longitude of point at end of strip
c                       (positive for western hemisphere)
c             ylfar     Latitude of point at end of strip
c                       (positive for northern hemisphere)
c
c
...
c ---    point in the strip lies on the eastern boundary of the sheet.
         ...
         xlnear = FLOAT(lonsedb)
         ylnear = FLOAT(latsedb) + stepj
         xlfar  = xlnear + 1.0
         ylfar  = ylnear
...
c ---    point in the strip lies on the southern boundary of the sheet.
         ...
         xlnear = FLOAT(lonsedb+1) - stepj
         ylnear = FLOAT(latsedb)
         xlfar  = xlnear
         ylfar  = ylnear + 1.0
...
c ---   Set dummy values for lat/lon (not used)
        xlnear = -999.
        ylnear = -999.
        xlfar  = -999.
        ylfar  = -999.
...
c ---   Add base elevation to heights (skip voids)
      ...
      elseif(L3SEC.or.LCDED) then

c ---    USGS-format Lat-Lon quadrangle sheet.
c ---    Data are ordered in strips of constant longitude,
c ---    starting with SW corner.
c ---    Load data for one strip of one sheet into data(np), where the
c ---    strip is oriented south-north (constant longitude), and the 1st
...
c ---   Define start and stop points of strip
        ...
        xfar  = xnw + (xne-xnw) * stepj
        yfar  = ynw + (yne-ynw) * stepj
c ---   9/9/14 fix: Canadian high resolution data come in 0.25 by 0.25 degree
c       DEM files, all other DEM files come in 1 by 1 degree sheets
        if(lcded.and.(hor_res.eq.0.75)) then
          xlnear = alonsedb+0.25-stepj
          ylnear = alatsedb
          xlfar  = xlnear
          ylfar  = ylnear + 0.25
        else
...
c ---    Per documentation lat,lon reference SE corner of 1-deg sheet
         xlnear = FLOAT(lon+1)
         ylnear = FLOAT(lat+1) - stepj
         xlfar  = xlnear - 1.0
         ylfar  = ylnear
c
...
c-----------------------------------------------------------------------
c
c --- TERREL    Version: 7.0.0          Level: 110330          LOADSRTM
c               K. Morrison
c
c
...
c
c --- Update
c     Ver. 3.66  Level 050817 to Ver. 3.69 Level 110330        KAM
c              - Use byte flipping only if the file and the machine
c                have different endians
c              - Add latitude/longitude near-far output        DGS
c              - Both x and y values of strips need to be incremented
c                by 1 since first strip is skipped in SETSRTM  DGS
...
c --- Find coordinates of strip, accounting for first strip that was
c --- skipped in SETSRTM
      stepj = (j-1) * stepb
      stepj2 = stepj + stepb

c --- Cartesian coordinates
c --- Why was this done in v3.687_draft?  Previous version had it right
c      xnear = xnw + (xsw-xnw) * stepj
c      ynear = ynw + (ynw-yne) * stepj2
c      xfar  = xne + (xse-xne) * stepj
c      yfar  = yne + (ysw-yse) * stepj2
c --- Revised
      xnear = xnw + (xsw-xnw) * stepj2
      ynear = ynw + (ysw-ynw) * stepj2
      xfar  = xne + (xse-xne) * stepj2
      yfar  = yne + (yse-yne) * stepj2

c --- NLat-WLon coordinates
c --- Per documentation strip is west-to-east, progressing
c --- north-to-south from the NW corner
      xlnear = FLOAT(lonsedb+1)
      ylnear = FLOAT(latsedb+1) - stepj2
      xlfar  = xlnear - 1.0
      ylfar  = ylnear

      if(lbigendian.eqv.lsrtmend) then
        read (ioinp) (data(i),i=1,npt+1) ! no need to swap
      else
        read (ioinp) (data1(i+1),data1(i),i=1,npt*2,2),data(npt+1)
c       To change data from "big endian" to "little endian" or 
c       vice-versa by swapping bytes
      end if
c
      return
      end
...
c-----------------------------------------------------------------------
c
c --- TERREL    Version: 7.0.0         Level: 970505           CELLIND
c               D. Strimaitis, SRC
c
c
...
c-----------------------------------------------------------------------
c
c --- TERREL    Version: 7.0.0         Level: 031017           CELLDAT
...
c-----------------------------------------------------------------------
c
c --- TERREL    Version: 7.0.0         Level: 031017            CELLIN
c
c ADAPTED from program ELEV in the ARM3 model
c               G. Lundberg, SAI
c         with enhancements by
...
c-----------------------------------------------------------------------
c
c --- TERREL    Version: 7.0.0         Level: 010713           CELLOUT
c
c ADAPTED from program ELEV in the ARM3 model
c               G. Lundberg, SAI
c         with enhancements by
...
c-----------------------------------------------------------------------
c
c --- TERREL    Version: 7.0.0         Level: 141010           HEIGHTS
c               D. Strimaitis, SRC
c
c
...
c
c --- Updates
c     Ver 3.66 Level 050817 to TNG-3.8.0 Level 141010 - DGS
c              Augment list-file documention of noise
c     Ver 3.65 Level 050815 to Ver 3.66 Level 050817 - KAM
...
21    format(a30,f8.1,4x,i5,6x,i5)
      ...
      write(iolst,*) nnoiset,' cells had noisy data'
        ...
        if(maxval(inoiserep).gt.0) then
          write(iolst,*) 'Elevations were reset to following values:'
          write(iolst,19)
     ...
     &                    htnoise(5),nnoise(5),nnoiser(5)
          ...
          write(iolst,*) 'No elevations were reset; noise tagged as',
     &                   ' any elevation less than (land minimum): ',
     &                    znoise(2)
...
c-----------------------------------------------------------------------
c
c --- TERREL    Version: 7.0.0         Level: 980304           FILLANG
c               D. Strimaitis, SRC
c
c
...
c-----------------------------------------------------------------------
c
c --- TERREL    Version: 7.0.0         Level: 990130             HCOPY
...
c-----------------------------------------------------------------------
c
c --- TERREL    Version: 7.0.0         Level: 980304            MODOUT
...
c-----------------------------------------------------------------------
c
c --- TERREL    Version: 7.0.0         Level: 020730            PLTOUT
c               D. Strimaitis   SRC
c
c
c PURPOSE:     Writes terrain elevation data (heights, m MSL) to
...
c----------------------------------------------------------------------
c
c --- TERREL    Version: 7.0.0         Level: 990130               OUT
c ---           J. Scire, SRC
c
...
c----------------------------------------------------------------------
c
c --- TERREL    Version: 7.0.0         Level: 901130               WRT
c ---           J. Scire, SRC
c
...
c----------------------------------------------------------------------
c
c --- TERREL    Version: 7.0.0         Level: 901130              WRT2
c ---          J. Scire, SRC
c
...
c-----------------------------------------------------------------------
c
c --- TERREL    Version: 7.0.0         Level: 031017            XYELEV
c               D. Strimaitis, SRC
c
...
c-----------------------------------------------------------------------
c
c --- TERREL    Version: 7.0.0         Level: 050817              XYIN
c               D. Strimaitis, SRC
c
...
c-----------------------------------------------------------------------
c
c --- TERREL    Version: 7.0.0         Level: 031031          XYOUTPUT
c               D. Strimaitis, SRC
c
...
c-----------------------------------------------------------------------
c
c --- TERREL    Version: 7.0.0          Level: 110330           GET4CNR
c ---           Shuming Du
c
...
c ---    Using Tangential TM grid
         ...
         write(iolst,*)' (Universal convention for longitude)'
c ---    Set conversion vectors
         call GLOBE1('TM      ',iutm,tmscaleo,xdum,xdum,rlat,rlon,
     &               feast,fnorth,
     &               'LL      ',iutm,xdum,xdum,xdum,rdum,rdum,
     &               feasti,fnorti,
     &               caction,vecti,vecto)
...
c----------------------------------------------------------------------
c
c --- TERREL    Version: 7.0.0         Level: 090511              FIN
c ---          J. Scire
c
...
c --- UPDATES
c
c     Ver 3.3 Level 021018 to Ver 3.3687 Level 090511     DGS
...
c-----------------------------------------------------------------------
c
c --- TERREL    Version: 7.0.0         Level: 050824           WRTHEAD
c               D. Strimaitis, Earth Tech
c
...
c-----------------------------------------------------------------------
c      
c --- TERREL    Version: 7.0.0         Level: 050817          LBIG_END
c               K. Morrison, Earth Tech
c
...
c-----------------------------------------------------------------------
c
c --- TERREL    Version: 7.0.0         Level: 031017           INTERP2D
c               K. Morrison, Earth Tech
c
c
...
c-----------------------------------------------------------------------
c
c --- TERREL    Version: 7.0.0         Level: 050526          CELLFILL
c               K. Morrison, Earth Tech
c
...
c-----------------------------------------------------------------------
      subroutine settrano(cmapi,iutmzni,tmscalei,xlat1i,xlat2i,rlati,
     &  rloni,feasti,fnorti,izone,tmscaleo,xlat1,xlat2,rlat,rlon,
     &  feast,fnorth,caction,vecti,vecto)
c-----------------------------------------------------------------------
c
c --- TERREL    Version: 7.0.0        Level: 110330            SETTRANO
c               K. Morrison
c
...
c
c Updates:
c         Version 3.687_draft Level 110330 from Version 3.686 Level 051201
c           - Add TMSCALEI as an input for input TM data
c           - Rename TMSONE to TMSCALEO for consistency
...
c         TMSCALEI - real       - Scale Factor for TM projection
...
c                                  (S. hemisphere is NEGATIVE)
c         TMSCALEO - real       - Scale Factor for TM projection
...
c --- Set translation vectors to the output grid
	...
	if(lutm) then
c ---    Using UTM grid
         call GLOBE1(cmapi,iutmzni,tmscalei,xlat1i,xlat2i,rlati,rloni,
     &               feasti,fnorti,
...
c ---    Using Tangential TM grid
         call GLOBE1(cmapi,iutmzni,tmscalei,xlat1i,xlat2i,rlati,rloni,
     &               feasti,fnorti,
     &               'TM      ',idum,tmscaleo,xdum,xdum,rlat,rlon,
     &               feast,fnorth,
     &               caction,vecti,vecto)
...
c-----------------------------------------------------------------------
c
c --- TERREL    Version: 7.0.0          Level: 050817           COASTRD 
c               K. Morrison  Earth Tech
c
c
...
c-----------------------------------------------------------------------
c
c --- TERREL    Version: 7.0.0          Level: 040228            IPTYPE
c               K. Morrison, Earth Tech
c
c
...
c-----------------------------------------------------------------------
c
c --- TERREL    Version: 7.0.0           Level: 141010           BREADH
c               K. Morrison
c
c
...
c
c --- Updates
c        Ver 3.687_draft Level 110330 to Ver TNG-3.8.0 Level 141010  DGS
c           - Revise GSHHG header processing for dataset v2.0 changes
c             (GSHHG 2.0 is denoted as version 10) 
c        Version 3.687_draft Level 110330 from Version 3.684 Level 041013
c           - Add decomposition of header variable #3 for version 1.4 of 
c             GSHHS file into the 4 variables previously separate.  il9
c             value will be 4 for the most recent GSHHS Version 1.5 (070402).
...
c --- Local variables
c      
      integer il1,il2,il3,il4,il5,il6,il7,il8,il9,il10,il11,il12
      integer ill1,ill2,ill3,ill4,ill5,ill6,ill7,ill8,ill9,
     &        ill10,ill11,ill12
      integer*2 is1,is2,ils1,ils2, is3
      integer*1 iwrk1
      integer*1 isl1(4),isl2(4),isl3(4),isl4(4),isl5(4),isl6(4),
     * isl7(4),isl8(4),isl9(4),isl10(4),isl11(4),isl12(4)
      integer*1 iss1(2),iss2(2)
      logical leof,lbigendian,lver3,lver4,lver10
      data lver3,lver4,lver10/.false.,.false.,.true./
      integer ilast/-1/
c      
c     Equivalences to allow byte-by-byte access
c
      equivalence (ils1,iss1(1)),(ils2,iss2(1))
      equivalence (ill1,isl1(1)),(ill2,isl2(1)),(ill3,isl3(1)),
     &            (ill4,isl4(1)),(ill5,isl5(1)),(ill6,isl6(1)),
     &            (ill7,isl7(1)),(ill8,isl8(1)),(ill9,isl9(1)),
     &            (ill10,isl10(1)),(ill11,isl11(1)),(ill12,isl12(1))
...
c       the read of the variables in the header based on the version
c
        if(lver10) then
c ---     Version number 10 appears to be v2.0
          read(iogshhs,err=99) il1,il2,ill3,il4,il5,il6,
     &                         il7,il8,il10,il11,il12
c ---     Decompose ill3
          il9=isl3(3)
          il3=isl3(4)
c ---     Use bit sequence for V2.0+ to decompose part of ill3
          iwrk1=0
          call MVBITS(isl3(2),0,1,iwrk1,0)
          is1=iwrk1
          iwrk1=0
          call MVBITS(isl3(4),0,1,iwrk1,0)
          is2=iwrk1
          iwrk1=0
          call MVBITS(isl3(4),1,1,iwrk1,0)
          is3=iwrk1
        elseif(lver4) then
          read(iogshhs,err=99) il1,il2,ill3,il4,il5,il6,il7,il8
          il9=isl3(3)
          il3=isl3(4)
          is1=isl3(2)
          is2=isl3(1)
...
c ---     Version number 10 appears to be v2.0
          read(iogshhs,err=99)
     *      isl1(4),isl1(3),isl1(2),isl1(1),
     *      isl2(4),isl2(3),isl2(2),isl2(1),
     *      isl3(1),isl3(2),isl3(3),isl3(4),
     ...
     *      isl10(4),isl10(3),isl10(2),isl10(1),
     *      isl11(4),isl11(3),isl11(2),isl11(1),
     *      isl12(4),isl12(3),isl12(2),isl12(1)
c ---     Decompose ill3
          il9=isl3(3)
          il3=isl3(4)
c ---     Use bit sequence for V2.0+ to decompose part of ill3
...
c !!! DEBUG
c           write(*,*)'Inside from BREADH: '
c           write(*,*)'  id         il1 = ',il1
c           write(*,*)'  npts       il2 = ',il2
c           write(*,*)'  itype      il3 = ',il3
c           write(*,*)'  ixmin      il4 = ',il4
c           write(*,*)'  ixmax      il5 = ',il5
c           write(*,*)'  iymin      il6 = ',il6
c           write(*,*)'  iymax      il7 = ',il7
c           write(*,*)'  iarea      il8 = ',il8
c           write(*,*)'  ivers      il9 = ',il9
c           write(*,*)'  igreen     is1 = ',is1
c           write(*,*)'  isource    is2 = ',is2
c           write(*,*)'  iriver     is3 = ',is3
c           write(*,*)'  iarea_full il10 = ',il10
c           write(*,*)'  icontainer il11 = ',il11
c           write(*,*)'  iancestor  il12 = ',il12

c
c     Test the GSHHS version number
c       - for Version 2.0, il9 must equal 10
c       - for Version 1.5, il9 must equal 4 or 5
c       - for Version 1.3, il9 must equal 3
c
      if(lver10.and.(il9.lt.10)) then
        lver10=.false.
        lver4=.true.
        lver3=.false.
        rewind (unit=iogshhs)
...
55      format(/' Note : GSHHS version is obsolete - update ASAP'//)
      elseif(lver4.and.(il9.lt.4)) then
        lver10=.false.
        lver4=.false.
        lver3=.true.
        rewind (unit=iogshhs)
        goto 1
      elseif(lver3.and.(il9.ne.3)) then
        lver10=.false.
        lver4=.false.
        lver3=.false.
        il9=3
...
c-----------------------------------------------------------------------
c
c --- TERREL    Version: 7.0.0          Level: 040228            BREADP
c               K. Morrison  Earth Tech
c
c
...
c-----------------------------------------------------------------------
c
c --- TERREL    Version: 7.0.0          Level: 141010          COASTEXT
c               K. Morrison
c
c
...
c         Ver 3.687 to Ver TNG-3.8.0 Level 141010
c              - Trap and report error opening files (not found)
c         Ver. 3.686 Level 051201 to Ver. 3.687_draft Level 110330 KAM
c              - Skip empty polygons (possibly added to most recent
c                GSHHS)
c         Ver. 3.67 Level 050817 to Ver 3.68 Level 051201       KAM
c              - Add additional dummy arguments in calls to SETTRANO to
c                be consistent with CTGPROC V. 2.65 Lev. 051201
...
c --- Set utm zone for coordinate calls
      ...
      call SETTRANO('LL      ',iutm,rdum,rdum,rdum,rdum,rdum,
     &            feasti,fnorti,
     &            iutm,tmscaleo,xlat1,xlat2,rlat,rlon,
     &            feast,fnorth,
     &            caction,vecti,vecto)
...
c
     ...
     &  status='OLD',recl=irecl,iostat=ierr)
      if(ierr.NE.0) call OPEN_ERR(iolst,'COASTEXT','Coastline File',
     &                            gshhsin,iogshhs)
c
...
c DGS
      write(iolst,*)'BIGENDIAN for GSHHS input: ',lbigendian
      write(iolst,*)
c DGS
...
c !!! DEBUG
c           write(iolst,*)'Returned from BREADH: '
c           write(iolst,*)'   id       il1 = ',id
c           write(iolst,*)'   npts     il2 = ',npts
c           write(iolst,*)'   itype    il3= ',itype
c           write(iolst,*)'   ixmin    il4= ',ixmin
c           write(iolst,*)'   ixmax    il5= ',ixmax
c           write(iolst,*)'   iymin    il6= ',iymin
c           write(iolst,*)'   iymax    il7= ',iymax
c           write(iolst,*)'   iarea    il8= ',iarea
c           write(iolst,*)'   igreen   is1= ',igreen
c           write(iolst,*)'   isource  is2= ',isource
...
c           from WDBII and logical not set or if it is not a polygon,
c           (i.e. river or border) simply read through to go
c           to next polygon
c
            if((xlonee.lt.xmini).or.(xlonww.gt.xmaxi)
     &        .or.(ylatnn.lt.ymini).or.(ylatss.gt.ymaxi).or.
     &        (isource.eq.1.and..not.lwdbii).or.(iarea.eq.0))
     &        then
               do j=1,npts
                 read(iogshhs) idum1,idum2
               enddo
c              Remove compiler warnings
               idum1=idum1
               idum2=idum2
...
c-----------------------------------------------------------------------
c
c --- TERREL    Version: 7.0.0          Level: 040311             ISIDE
c               K. Morrison  Earth Tech
c
c
...
c-----------------------------------------------------------------------
c
c --- TERREL    Version: 7.0.0          Level: 040228           PCORNER
c               K. Morrison  Earth Tech
c
c
...
c-----------------------------------------------------------------------
      subroutine setgeotiff(k)
c-----------------------------------------------------------------------
c
c --- TERREL    Version: 7.0.0          Level: 110330        SETGEOTIFF
c               K. Morrison
c
c PURPOSE:     SETGEOTIFF sets the logicals and opens the GeoTIFF file.
c
c --- Update
c     Ver 3.69 to Ver TNG-3.8.0 Level 141010
c              - Trap and report error opening files (not found)
...
c-----------------------------------------------------------------------
c
c --- TERREL    Version: 7.0.0           Level: 140912         LOADTIFF
c               K. Morrison
c
c
c PURPOSE:     LOADTIFF reads the GeoTIFF file information, sets 
c              translation parameters, reads the data in the
c              database file, and passes to SAVEELEV the intermediate
c              data for the discrete receptors and grid cells to which
c              they apply.
c
c --- Update
c
c --- version 3.69 Level 110330 to version 3.7.0 Level 140829 (C.Escoffier)
c              the geotiff utm zone was entered as always positive (izone) 
c              in subroutine settrano. It is converted as positive or negative
c              depending on the hemisphere as a variable called iutm
c              iutm now replaced izone in the settrano call to get projection
c              information for the input geotiff data
c
c
c --- INPUTS  /CONTROL/ program control data
c             /CORNR/   corners of sheet of terrain data
...
c CALLING ROUTINES:   COMP
c
c EXTERNAL ROUTINES:  GET_IFD, READTIFF, SETTRANO, GLOBE, SAVEELEV
c-----------------------------------------------------------------------
c --- Include file of parameters and commons
      include 'params.trl'
      include 'control.trl'
      ...
      real*8 vecti(9),vecto(9)
      character*12 caction
      character*4 c4dum,utmhemi
      character*8 cmapi/'LL      '/
      real rdata(mxnp),feasti/0./,fnorti/0./
      real xkm(4),ykm(4)

      data xdum/0.0/

      open(unit=500,file='numpts.dat')
c --- Set utm zone for coordinate calls
      iutm=izone
      if(utmhem.EQ.'S   ' .AND. izone.LT.900) iutm=-iutm

c      write(*,*) dbdatum,cmapi
      call get_ifd(ioinp,nxi,nyi,dxi,dyi,dbdatum,rlati,rloni,xlat1i,
     &     xlat2i,feasti,fnorthi,cmapi,xorg,yorg,utmhemi,iutmzni,
     &     dbtmsf,itifftype,.false.)
      xine=xorg+(nxi-1)*dxi
      yinn=yorg+(nyi-1)*dyi

      write(iolst,*)
      write(iolst,*) '-----------------------------'
      write(iolst,*) 'GeoTIFF Data File Information'
      write(iolst,*) '-----------------------------'
      write(iolst,*)
      write(iolst,*) 'File Datum      = ',dbdatum
      write(iolst,*) 'File Projection = ',cmapi
      write(iolst,*) 'X-origin        = ',xorg
      write(iolst,*) 'Y-origin        = ',yorg
      write(iolst,*) 'Spacing (X,Y)   = ',dxi,dyi
      write(iolst,*)

      if(utmhemi.EQ.'S   ') iutmzni=-iutmzni
      
      if(itifftype.ne.1) then
         write(*,*) 'Invalid GeoTIFF type - pixels must be values'
         stop
      endif

c --- izone does not have a sign as it is entered with a S or N to
c --- indicate North of South.
c --- iutm is a signed integer that is negative in the S. Hem.
c --- Use iutm in the call to settrano (cec - 20140829)
c      call settrano(cmapi,iutmzni,dbtmsf,xlat1i,xlat2i,rlati,
c     &  rloni,feasti,fnorthi,izone,tmscaleo,xlat1,xlat2,rlat,rlon,
c     &  feast,fnorth,caction,vecti,vecto)
      call settrano(cmapi,iutmzni,dbtmsf,xlat1i,xlat2i,rlati,
     &  rloni,feasti,fnorthi,iutm,tmscaleo,xlat1,xlat2,rlat,rlon,
     &  feast,fnorth,caction,vecti,vecto)
...
c --- Determine coordinates of the 4 corners of the input file
      call GLOBE(iolst,caction,dbdatum,vecti,datum,vecto,
     &             xorg,yorgt,xkm(1),ykm(1),idum,c4dum)
      call GLOBE(iolst,caction,dbdatum,vecti,datum,vecto,
     &             xine,yorgt,xkm(2),ykm(2),idum,c4dum)
      call GLOBE(iolst,caction,dbdatum,vecti,datum,vecto,
     &             xine,yinn,xkm(3),ykm(3),idum,c4dum)
      call GLOBE(iolst,caction,dbdatum,vecti,datum,vecto,
     &             xorg,yinn,xkm(4),ykm(4),idum,c4dum)
...
c --- Set index range for x, y
      ...
      dxkm=(min(abs(xkm(2)-xkm(1)),abs(xkm(3)-xkm(4))))/real(nxi)
      dykm=(min(abs(ykm(4)-ykm(1)),abs(ykm(3)-ykm(2))))/real(nyi)

      nbegx=int((xllk-max(xkm(1),xkm(4)))/dxkm)
      nendx=int((xurk-min(xkm(1),xkm(4)))/dxkm)+1
      nbegy=int((min(ykm(3),ykm(4))-yurk)/dykm)
      nendy=int((max(ykm(3),ykm(4))-yllk)/dykm)+1

c --- Apply 10% buffer to range
      nbuffx=abs(nendx-nbegx)/10 + 1
      nbuffy=abs(nendy-nbegy)/10 + 1
      nbuff=MAX(nbuffx,nbuffy)
      nbegx=nbegx-min(nbuff,nbegx-1)
      nendx=nendx+min(nbuff,nxi-nendx)
      nbegy=nbegy-min(nbuff,nbegy-1)
      nendy=nendy+min(nbuff,nyi-nendy)
...
c --- Determine absolute spacing, in meters divided by 2
      dels = (SQRT(dxkm*dxkm + dykm*dykm))*0.5e+3
      if(LPOLR) then
c ---    POLAR - spacing in meters
      else
c ---    CARTESIAN - spacing in grid units
...
c --- Flip begining and ending y and y origin since GeoTIFF is top to bottom
...
c ---       Translate DB to grid coordinates
            call GLOBE(iolst,caction,dbdatum,vecti,datum,vecto,
...
c ---       Read record for this cell
            kcnt = (j-1)*nxi +i
            call readtiff(ioinp,kcnt,lcc1,elev,ipos)
            if(elev.gt.20000..or.elev.lt.-10000.) elev=0.

c ---       Save discrete and cell data
            call SAVEELEV(xdist,ydist,dels,elev)
...
999   continue
      WRITE(iolst,*) 'GeoTIFF DATA BASE DOES NOT CONTAIN GRID AREA'
      lnotgrid = .TRUE.
      if(lnotgrid) notused = notused + 1
c
      return
